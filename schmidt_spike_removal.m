...
function despiked_signal= schmidt_spike_removal(original_signal, fs, median_multi,windowsize)
original_signal = original_signal(end:-1:1);
...
count=0;
while(~isempty(find((MAAs>median(MAAs)*median_multi)))&&count<128)
    ...
    spike_position = find(abs(sampleframes(:,window_num))>median(MAAs)*median_multi);

    spike_start =spike_position(1);
    spike_end = spike_position(end);
...
%     end
    ...
    %Set to Zero
    ind_start=windowsize*(window_num-1)+(spike_start-1-(spike_end-spike_start));
    ind_end = windowsize*(window_num-1)+spike_start-1;
    if windowsize*(window_num-1)+(spike_start-1-(spike_end-spike_start))<1
     sampleframes(spike_start:spike_end,window_num) = mean(abs(original_signal));
    else
    sampleframes(spike_start:spike_end,window_num) = original_signal(ind_start:ind_end);
    end
    
%      sampleframes(spike_start:spike_end,window_num) = mean(abs(original_signal));
   
%     if spike_end-spike_start>200
%         long_spike=1;
...
despiked_signal = despiked_signal(end:-1:1);
